    ...
    <v-dialog v-model="dialog" persistent max-width="600px">
      <template v-slot:activator="{ on, attrs }">
        <v-btn color="primary" dark v-bind="attrs" v-on="on">
          <v-icon class="mr-2">mdi-plus</v-icon><span>New Form</span>
        </v-btn>
      </template>
      <v-card>
        <v-card-title>
          <span class="headline">New Form</span>
        </v-card-title>
        <v-card-text>
          <v-container>
...
export default {
  ...
  watch: {
    ...
    form_addFormLoading(val) {
      if (!val) {
        if (!this.form_addFormError) {
          this.dialog = false;
          this.$v.$reset();
          this.resetAll();
        }
  ...
  computed: {
    ...mapGetters(["form_addFormLoading", "form_addFormError"]),
